...
HANDLE CVodStreamCache::Create(char token[], char path[], char savePath[])
{
	// 出错了
	if (err)
	{
	...
	// 创建指定大小的文件
	cacheFile.open(savePath, ios::binary|ios::out);
...
{
	cacheFile.open("E:\\trim-000015.dat", ios::binary|ios::in);
	if (!cacheFile)
	{
...
int CVodStreamCache::WriteBlock(long offset, int len, char *buff,char savePath[])
{
	...
	cacheFile.open(savePath, ios::binary|ios::in|ios::out);
	if (!cacheFile)
	{
		cout << "cannot open file" << endl;
	}
	...
	cacheFile.close();
	if ((cache.getTotalLen() - offset) / 1024 == 0)
		rcv_bytes = 0;
	return rcv_bytes;
}
